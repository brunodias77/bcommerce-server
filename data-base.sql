...
CREATE INDEX idx_product_sale_dates ON product (sale_price_start_date, sale_price_end_date)
...
CREATE TABLE product_variant (
    ...
    color_id INTEGER REFERENCES color(color_id) ON DELETE RESTRICT,
    size_id INTEGER REFERENCES size(size_id) ON DELETE RESTRICT,
    stock_quantity INTEGER NOT NULL DEFAULT 0 CHECK (stock_quantity >= 0),
...
-- UNIQUE parcial (soft delete-aware)
CREATE UNIQUE INDEX uq_active_variant_attributes
...
-- UNIQUE parcial (soft delete-aware)
CREATE UNIQUE INDEX uq_active_pav
ON product_attribute_value (product_id, product_attribute_id)
WHERE deleted_at IS NULL;
...
-- UNIQUE parcial (soft delete-aware)
CREATE UNIQUE INDEX uq_active_pt
...
CREATE TABLE "order" (
    ...
    coupon_id INTEGER REFERENCES coupon(coupon_id) ON DELETE SET NULL,
...
CREATE TABLE order_item (
    ...
    product_variant_id INTEGER REFERENCES product_variant(product_variant_id) ON DELETE SET NULL,
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT chk_order_item_link CHECK (
        product_id IS NOT NULL OR
        product_variant_id IS NOT NULL OR
        (item_sku IS NOT NULL AND item_name IS NOT NULL)
    )
);
...
-- Validação opcional de JSONB (poderia ser validada via app ou pg_jsonschema)
-- Exemplo básico de formato (substitua se necessário):
-- ALTER TABLE payment ADD CONSTRAINT chk_method_details_format CHECK (
--     jsonb_typeof(method_details) = 'object' OR method_details IS NULL
-- );
...
CREATE TABLE coupon_usage (
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT uq_coupon_usage_order_active UNIQUE (order_id) WHERE deleted_at IS NULL
);
...
CREATE TABLE address (
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT uq_address_details_per_client_type UNIQUE (client_id, type, postal_code, street, number, complement, country_code)
);

-- UNIQUE por tipo de endereço e padrão por cliente
CREATE UNIQUE INDEX uq_address_default_shipping_per_client
    ON address (client_id) WHERE type = 'shipping' AND is_default = TRUE AND deleted_at IS NULL;

CREATE UNIQUE INDEX uq_address_default_billing_per_client
    ON address (client_id) WHERE type = 'billing' AND is_default = TRUE AND deleted_at IS NULL;
...
-- ================================================
-- TABELA: address_history (imutável)
-- ================================================
...
CREATE TABLE order_address (
    ...
    country_code CHAR(2) NOT NULL DEFAULT 'BR',
    phone VARCHAR(20),
    original_address_id INTEGER REFERENCES address(address_id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
...
CREATE UNIQUE INDEX uq_active_oa_type
ON order_address (order_id, address_type) WHERE deleted_at IS NULL;
...
CREATE TABLE cart_item (
    ...
    product_variant_id INTEGER REFERENCES product_variant(product_variant_id) ON DELETE CASCADE,
    quantity INTEGER NOT NULL CHECK (quantity > 0),
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT chk_cart_item_link CHECK (
        product_id IS NOT NULL OR product_variant_id IS NOT NULL
    )
);

-- Unicidade condicional por tipo de item
CREATE UNIQUE INDEX uq_cart_item_active_product
    ON cart_item (cart_id, product_id)
    WHERE product_variant_id IS NULL AND product_id IS NOT NULL AND deleted_at IS NULL;

CREATE UNIQUE INDEX uq_cart_item_active_variant
...
CREATE TABLE favorite (
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT uq_favorite_client_product UNIQUE (client_id, product_id)
);

-- UNIQUE parcial (soft delete-aware)
CREATE UNIQUE INDEX uq_fav_active
    ON favorite (client_id, product_id) WHERE deleted_at IS NULL;
...
-- Indexação condicional para manter unicidade lógica com soft delete
CREATE UNIQUE INDEX uq_review_active_client_product
    ON review (client_id, product_id)
    WHERE client_id IS NOT NULL AND deleted_at IS NULL;

CREATE UNIQUE INDEX uq_review_active_anonymous_product
...
CREATE TABLE consent (
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT uq_client_consent_subject_version UNIQUE (client_id, type, consent_subject, terms_version)
);

-- UNIQUE parcial (soft delete-aware)
CREATE UNIQUE INDEX uq_active_consent
    ON consent (client_id, type, consent_subject, terms_version)
    WHERE deleted_at IS NULL;
...
CREATE TABLE saved_card (
    ...
    deleted_at TIMESTAMPTZ,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT uq_saved_card_client_gateway_token UNIQUE (client_id, gateway_token)
);
...
CREATE TABLE audit_log (
    ...
    retention_expires_at TIMESTAMPTZ DEFAULT (CURRENT_TIMESTAMP + INTERVAL '2 years')
);
